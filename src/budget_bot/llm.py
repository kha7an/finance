...
def build_prompt(category_book: CategoryBook, screenshot_date: date, screenshot_count: int = 1) -> str:
    if screenshot_count > 1:
        batch_context = {
            ...
            "rules": [
                f"Это пачка из {screenshot_count} скриншотов, отправленных одним Telegram-альбомом.",
                "Считай их частями одной непрерывной истории операций в порядке отправки.",
                (
                    "Если на следующем скриншоте нет нового заголовка дня, это продолжение предыдущего "
                    "видимого дня, а не следующий календарный день."
                ),
                (
                    "Если дата дня видна на одном из скриншотов пачки, применяй эту дату ко всем последующим "
                    "операциям пачки, пока не появится новый видимый заголовок дня. В таком случае ставь "
                    '"date_status": "relative".'
                ),
                (
                    "Если в начале не первого скриншота пачки видны операции без заголовка дня, продолжай дату "
                    "с предыдущего скриншота пачки."
                ),
                (
                    "Если это первый скриншот пачки и операции идут выше первого видимого заголовка дня, "
                    "не извлекай эти верхние операции."
                ),
                ...
                (
                    "Если одинаковые операции действительно видны несколько раз подряд с тем же названием "
                    "и суммой, верни их отдельными элементами."
                ),
            ],
        }
    else:
        batch_context = {
            ...
            "rules": [
                (
                    "Это один отдельный скриншот, не альбом. Если операции видны выше первого видимого "
                    "заголовка дня, и у них нет собственной видимой даты, не добавляй эти верхние операции "
                    "в operations."
                )
            ],
        }
    ...
    instruction = {
        "task": "Извлеки операции из скриншота истории банковских операций.",
        "output": {
            "format": "JSON only",
            "markdown": False,
            "language": "ru",
        ...
        "date_rules": [
            ...
            (
                'Если виден заголовок дня вроде "1 августа", эта дата относится ко всем операциям ниже '
                "до следующего заголовка дня, даже если нижние строки далеко от заголовка."
            ),
            "Заголовок дня никогда не относится к операциям выше него.",
            (
                "Когда появляется следующий заголовок дня ниже, предыдущая дата больше не действует "
                "для операций под новым заголовком."
            ),
            "Возвращай операции в том же порядке, как они идут на экране сверху вниз.",
            (
                'Если операция находится под видимым заголовком дня, используй дату этого заголовка и '
                '"date_status": "relative", а не "missing".'
            ),
            (
                'Если дата операции на скриншоте не видна и нет "Сегодня"/"Вчера"/другого явного '
                'указателя даты, не угадывай дату: верни "date": null и "date_status": "missing".'
            ),
        ],
        "allowed_categories": {
            "expenses": category_book.expense_categories,
            "incomes": category_book.income_categories,
        },
        "extraction_rules": [
            ...
            (
                "Если текст не удается надежно прочитать, не придумывай значение; поставь null "
                "для категории/подкатегории или needs_review=true."
            ),
            ...
            (
                'Зеленые входящие поступления от людей в разделе "Переводы" тоже помечай '
                'type="transfer", а не income.'
            ),
            (
                "Для transfer сохраняй направление: исходящий перевод - отрицательная сумма, "
                "входящий перевод - положительная сумма."
            ),
            (
                'Серые суммы справа от заголовка дня ("7 августа", "8 августа") - это итог дня, '
                "а не операция: никогда не используй их как amount."
            ),
            (
                "Даже если серый итог дня совпадает с суммой единственной операции под заголовком, "
                "верни только строку операции; сам серый итог дня не добавляй в operations."
            ),
            "Для операции бери сумму только из той же строки, что и название операции.",
            (
                "Кэшбэк, бонусы и желтые бонусные бейджи рядом с операцией не являются отдельными "
                "операциями: не добавляй их в operations."
            ),
            'Если строка явно называется "Кэшбэк", "Бонусы" или похожим образом, верни type="ignore".',
            (
                "Возвраты денег по покупкам возвращай как income, только если это отдельная зеленая "
                "операция возврата, а не бонусный бейдж."
            ),
            "Непонятные операции помечай type=\"ignore\" или верни category/subcategory null.",
            (
                "Если банк явно показывает категорию операции под названием магазина, используй ее "
                "как сильный сигнал при выборе категории и подкатегории из разрешенного списка."
            ),
            (
                "Не копируй банковскую категорию напрямую, если такого значения нет в разрешенном "
                "справочнике; сопоставь ее с ближайшей допустимой категорией."
            ),
            "Фастфуд вроде Бургер Кинг, KFC, Вкусно и точка относить к Еда / Фастфуд, если такая подкатегория есть.",
            (
                "Супермаркеты и продуктовые магазины вроде Пятерочка, Магнит, SPAR, Перекресток "
                "относить к Еда / Супермаркеты."
            ),
            ...
            (
                "Разрешается убрать технические префиксы, организационно-правовую форму и очевидный "
                "служебный мусор только если смысл названия не меняется."
            ),
            "Не придумывай полное название организации по сокращению.",
            (
                "needs_review=true, если сумма читается неоднозначно, дата неоднозначна, категория "
                "не определяется или название операции существенно обрезано."
            ),
        ],
        "response_contract": {
            ...
            "operations": [
                {
                    ...
                    "category": "Еда or null",
                    "subcategory": "Супермаркеты or null",
                    "needs_review": False,
                }
            ],
        },
    ...
    return (
        "Ты анализируешь банковские скриншоты. Выполни инструкцию ниже и верни только JSON ответа, "
        "соответствующий response_contract. Не возвращай JSON самой инструкции.\n\n"
