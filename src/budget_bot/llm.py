...
Заголовок дня никогда не относится к операциям выше него. Если в верхней части скриншота видны обрезанные операции
до первого видимого заголовка дня, не назначай им дату первого заголовка ниже; верни date=null и date_status="missing".
Когда появляется следующий заголовок дня ниже, предыдущая дата больше не действует для операций под новым заголовком.
...
- Зеленые входящие поступления от людей в разделе "Переводы" тоже помечай type="transfer", а не income.
...
- Даже если серый итог дня совпадает с суммой единственной операции под заголовком, верни только строку операции; сам серый итог дня не добавляй в operations.
